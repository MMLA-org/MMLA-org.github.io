...
这是 MMLA 研究仓库的零依赖静态主页。页面、样式、交互和随仓库发布的公开稿、完整技术报告与理论分支 PDF 都位于 `docs/`，不需要 Node、构建工具或第三方 CDN。

## 部署到 GitHub Pages

仓库已经包含 `.github/workflows/pages.yml`。将仓库推送到 GitHub 后：

1. 打开仓库的 **Settings → Pages**。
2. 将 **Source** 设为 **GitHub Actions**。
3. 推送到 `main` 分支，或在 Actions 页面手动运行 **Deploy MMLA GitHub Pages**。

工作流会把 `docs/` 作为 GitHub Pages 制品发布。也可以在本地直接打开 `docs/index.html`，或用任意静态文件服务器预览。

## 当前公开内容

主页和 `docs/papers/` 采用 `public_release_staging/mmla-memory/` 中的最新公开文件：
...
引用区按 `public_release_staging/mmla-memory/README.md` 的 BibTeX 整理，包含公开稿、正式技术报告和五个理论后续。若未来公开材料发生变化，应同时更新 `docs/papers/`、页面卡片和引用说明。
...
- `papers/`：随仓库发布的公开稿、正式技术报告与五份配套文档
